...
/// Write current worker state to `.ruoyi/worker-state.json` under the worker's cwd.
/// This is the file-based observability surface: external observers (ruoyi-orchestrator, orchestrators)
...
fn emit_state_file(worker: &Worker) {
    ...
    struct StateSnapshot<'a> {
        ...
        updated_at: u64,
        /// Seconds since last state transition. Ruoyihip uses this to detect
        /// stalled workers without computing epoch deltas.
        seconds_since_update: u64,
    }

    let state_dir = std::path::Path::new(&worker.cwd).join(".ruoyi");
...
mod tests {
    ...
    fn emit_state_file_writes_worker_status_on_transition() {
        ...
        let state_path = cwd_path.join(".ruoyi").join("worker-state.json");
        assert!(
            state_path.exists(),
