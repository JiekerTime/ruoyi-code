...
pub const RUOYI_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".ruoyi.json"),
            |parent| parent.join(".ruoyi.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".ruoyi.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".ruoyi").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".ruoyi").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("RUOYI_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".ruoyi")))
        .unwrap_or_else(|| PathBuf::from(".ruoyi"))
}
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".ruoyi.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, RUOYI_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".ruoyi");
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".ruoyi");
        fs::create_dir_all(cwd.join(".ruoyi")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".ruoyi.json"),
        ...
        assert_eq!(RUOYI_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".ruoyi");
        let project_settings = cwd.join(".ruoyi").join("settings.json");
        fs::create_dir_all(cwd.join(".ruoyi")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
