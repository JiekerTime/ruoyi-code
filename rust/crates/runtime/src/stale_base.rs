...
/// Read the `.ruoyi-base` file from the given directory and return the trimmed
/// commit hash, or `None` when the file is absent or empty.
pub fn read_ruoyi_base_file(cwd: &Path) -> Option<String> {
    let path = cwd.join(".ruoyi-base");
...
/// fall back to reading `.ruoyi-base` from `cwd`.
pub fn resolve_expected_base(flag_value: Option<&str>, cwd: &Path) -> Option<BaseCommitSource> {
    ...
    read_ruoyi_base_file(cwd).map(BaseCommitSource::File)
}
...
mod tests {
    ...
    #[test]
    fn reads_ruoyi_base_file() {
        // given
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".ruoyi-base"), "abc1234def5678\n").expect("write .ruoyi-base");

        // when
        let value = read_ruoyi_base_file(&root);
    ...
    #[test]
    fn returns_none_for_missing_ruoyi_base_file() {
        // given
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
    ...
    #[test]
    fn returns_none_for_empty_ruoyi_base_file() {
        // given
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".ruoyi-base"), "  \n").expect("write empty .ruoyi-base");
    ...
    fn resolve_expected_base_prefers_flag_over_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".ruoyi-base"), "from_file\n").expect("write .ruoyi-base");
    ...
    #[test]
    fn matches_with_ruoyi_base_file_in_real_repo() {
        // given
        let root = temp_dir();
        init_repo(&root);
        let sha = head_sha(&root);
        fs::write(root.join(".ruoyi-base"), format!("{sha}\n")).expect("write .ruoyi-base");
        let source = resolve_expected_base(None, &root);
    ...
    #[test]
    fn diverged_with_ruoyi_base_file_after_new_commit() {
        // given
        let root = temp_dir();
        init_repo(&root);
        let old_sha = head_sha(&root);
        fs::write(root.join(".ruoyi-base"), format!("{old_sha}\n")).expect("write .ruoyi-base");
