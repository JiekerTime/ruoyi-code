...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    // DeepSeek models. Uses the OpenAI-compatible REST shape.
...
pub fn detect_provider_kind(model: &str) -> ProviderKind {
    ...
    // DeepSeek is the default provider — check it first.
...
pub fn model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    ...
    match canonical.as_str() {
        ...
        // DeepSeek models: skip client-side context window check, let the server decide.
        _ => None,
    }
}
...
/// Look up `key` in `.env` files. Searches in order:
/// 1. Current working directory `.env`
/// 2. Global `~/.ruoyi-cli/.env`
///
/// Returns `None` when no file contains the key or the value is empty.
pub(crate) fn dotenv_value(key: &str) -> Option<String> {
    ...
    // Fall back to global ~/.ruoyi-cli/.env
    std::env::var_os("HOME")
        .map(std::path::PathBuf::from)
        .and_then(|home| load_dotenv_file(&home.join(".ruoyi-cli").join(".env")))
        .and_then(|values| values.get(key).filter(|v| !v.is_empty()).cloned())
}
...
mod tests {
    ...
    fn qwen_prefix_routes_to_dashscope_not_anthropic() {
        // User request from Discord #ruoyicode-get-help: web3g wants to use
    ...
    fn plugin_config_max_output_tokens_overrides_model_default() {
        ...
        std::fs::create_dir_all(cwd.join(".ruoyi")).expect("project config dir");
