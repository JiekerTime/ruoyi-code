...
const MANIFEST_RELATIVE_PATH: &str = ".ruoyi-plugin/plugin.json";
...
fn detect_claude_code_manifest_contract_gaps(
    ...
    for (field, detail) in [
        (
            "skills",
            "plugin manifest field `skills` uses the Claude Code plugin contract; `ruoyi` does not load plugin-managed skills and instead discovers skills from local roots such as `.ruoyi/skills`, `.omc/skills`, `.agents/skills`, `~/.omc/skills`, and `~/.ruoyi/skills/omc-learned`.",
        ),
        (
            "mcpServers",
            "plugin manifest field `mcpServers` uses the Claude Code plugin contract; `ruoyi` does not import MCP servers from plugin manifests.",
        ),
        (
            "agents",
            "plugin manifest field `agents` uses the Claude Code plugin contract; `ruoyi` does not load plugin-managed agent markdown catalogs from plugin manifests.",
        ),
    ] {
    ...
    {
        errors.push(PluginManifestValidationError::UnsupportedManifestContract {
            detail: "plugin manifest field `commands` uses Claude Code-style directory globs; `ruoyi` slash dispatch is still built-in and does not load plugin slash command markdown files.".to_string(),
        });
    }
    ...
    if let Some(hooks) = root.get("hooks").and_then(Value::as_object) {
        for hook_name in hooks.keys() {
            if !matches!(
                ...
                errors.push(PluginManifestValidationError::UnsupportedManifestContract {
                    detail: format!(
                        "plugin hook `{hook_name}` uses the Claude Code lifecycle contract; `ruoyi` plugins currently support only PreToolUse, PostToolUse, and PostToolUseFailure."
                    ),
                });
            }
        }
