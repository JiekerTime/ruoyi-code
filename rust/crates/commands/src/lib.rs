...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Ruoyi Code plugins",
        argument_hint: Some(
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    if let Ok(ruoyi_config_home) = env::var("RUOYI_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserClawConfigHome,
            PathBuf::from(ruoyi_config_home).join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".ruoyi").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".ruoyi").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(ruoyi_config_home) = env::var("RUOYI_CONFIG_HOME") {
        let ruoyi_config_home = PathBuf::from(ruoyi_config_home);
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClawConfigHome,
            ruoyi_config_home.join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClawConfigHome,
            ruoyi_config_home.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClaude,
            home.join(".ruoyi").join("skills").join("omc-learned"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Ok(ruoyi_config_home) = env::var("RUOYI_CONFIG_HOME") {
        return Ok(PathBuf::from(ruoyi_config_home).join("skills"));
    }
    if let Ok(codex_home) = env::var("CODEX_HOME") {
        return Ok(PathBuf::from(codex_home).join("skills"));
    }
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".ruoyi").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set RUOYI_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       ruoyi agents".to_string(),
        "  Sources          .ruoyi/agents, ~/.ruoyi/agents, $RUOYI_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "ruoyi-cli agents [list|help]",
            "sources": [".ruoyi/agents", "~/.ruoyi/agents", "$RUOYI_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       ruoyi skills [list|install <path>|help|<skill> [args]]".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $RUOYI_CONFIG_HOME/skills or ~/.ruoyi/skills".to_string(),
        "  Sources          .ruoyi/skills, .omc/skills, .agents/skills, .codex/skills, .ruoyi/skills, ~/.ruoyi/skills, ~/.omc/skills, ~/.ruoyi/skills/omc-learned, ~/.codex/skills, ~/.ruoyi/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "ruoyi-cli skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$RUOYI_CONFIG_HOME/skills or ~/.ruoyi/skills",
            "sources": [
                ".ruoyi/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       ruoyi mcp [list|show <server>|help]".to_string(),
        "  Sources          .ruoyi/settings.json, .ruoyi/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "ruoyi-cli mcp [list|show <server>|help]",
            "sources": [".ruoyi/settings.json", ".ruoyi/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
fn definition_source_id(source: DefinitionSource) -> &'static str {
    match source {
        DefinitionSource::ProjectClaw
        | DefinitionSource::ProjectCodex
        | DefinitionSource::ProjectClaude => "project_ruoyi",
        DefinitionSource::UserClawConfigHome | DefinitionSource::UserCodexHome => {
            "user_ruoyi_config_home"
        }
...
mod tests {
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Ruoyi Code plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn lists_agents_from_project_and_user_roots() {
        ...
        let user_agents = user_home.join(".ruoyi").join("agents");
    ...
    fn renders_agents_reports_as_json() {
        ...
        assert_eq!(report["agents"][2]["shadowed_by"]["id"], "project_ruoyi");

        let help = handle_agents_slash_command_json(Some("help"), &workspace).expect("agents help");
        assert_eq!(help["kind"], "agents");
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["direct_cli"], "ruoyi-cli agents [list|help]");
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".ruoyi").join("commands");
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".ruoyi").join("skills");
        let legacy_commands = workspace.join(".ruoyi").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(report["skills"][0]["source"]["id"], "project_ruoyi");
        assert_eq!(report["skills"][1]["name"], "deploy");
        assert_eq!(report["skills"][1]["origin"]["id"], "legacy_commands_dir");
        assert_eq!(report["skills"][3]["shadowed_by"]["id"], "project_ruoyi");
        ...
        assert_eq!(
            ...
            "ruoyi-cli skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       ruoyi agents"));
        ...
        assert!(
            skills_help.contains("Install root     $RUOYI_CONFIG_HOME/skills or ~/.ruoyi/skills")
        );
        assert!(skills_help.contains(".omc/skills"));
        assert!(skills_help.contains(".agents/skills"));
        assert!(skills_help.contains("~/.ruoyi/skills/omc-learned"));
        ...
        assert!(sources
            .iter()
            .any(|value| value == "~/.ruoyi/skills/omc-learned"));
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       ruoyi mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".ruoyi")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".ruoyi").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".ruoyi").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".ruoyi/settings.json");
