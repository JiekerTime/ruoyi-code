...
fn agents_command_emits_structured_agent_entries_when_requested() {
    ...
    assert_eq!(parsed["agents"][0]["source"]["id"], "project_ruoyi");
    ...
    assert_eq!(parsed["agents"][2]["shadowed_by"]["id"], "project_ruoyi");
}
...
fn assert_json_command_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Value {
    let output = run_ruoyi(current_dir, args, envs);
    assert!(
...
fn run_ruoyi(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
...
fn write_upstream_fixture(root: &Path) -> PathBuf {
    let upstream = root.join("ruoyi-code");
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "ruoyi-output-format-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
