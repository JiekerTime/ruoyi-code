...
fn status_command_applies_model_and_permission_mode_flags() {
    ...
    // when
    let output = Command::new(env!("CARGO_BIN_EXE_ruoyi-cli"))
...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_ruoyi-cli"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("ruoyi should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_ruoyi-cli"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("ruoyi should launch");
...
fn omc_namespaced_slash_commands_surface_a_targeted_compatibility_hint() {
    ...
    assert!(stderr.contains("unknown slash command outside the REPL: /ruoyi-plugin:hud"));
    assert!(stderr.contains("Ruoyi Code plugin command"));
...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    let config_home = temp_dir.join("home").join(".ruoyi");
    fs::create_dir_all(temp_dir.join(".ruoyi")).expect("project config dir should exist");
    ...
    fs::write(temp_dir.join(".ruoyi.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".ruoyi").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = command_in(&temp_dir)
        .env("RUOYI_CONFIG_HOME", &config_home)
        .args([
            "--resume",
            session_path.to_str().expect("utf8 path"),
            "/config",
    ...
    assert!(stdout.contains(temp_dir.join(".ruoyi.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".ruoyi")
...
fn doctor_command_runs_as_a_local_shell_entrypoint() {
    ...
    let output = command_in(&temp_dir)
        ...
        .arg("doctor")
        .output()
        .expect("ruoyi-cli doctor should launch");
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            ruoyi doctor"));
    ...
    assert!(status_stdout.contains("Usage            ruoyi status"));
...
fn command_in(cwd: &Path) -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_ruoyi-cli"));
