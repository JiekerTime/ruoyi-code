...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  ruoyi --resume"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".ruoyi")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".ruoyi").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    .expect("local config should write");

    // when
    let output = run_ruoyi_with_env(
        &project_dir,
        &[
            "--resume",
            session_path.to_str().expect("utf8 path"),
...
fn resume_latest_restores_the_most_recent_managed_session() {
    ...
    let project_dir = temp_dir.join("project");
    let sessions_dir = project_dir.join(".ruoyi").join("sessions");
    ...
    // when
    let output = run_ruoyi(&project_dir, &["--resume", "latest", "/status"]);
...
fn run_ruoyi(current_dir: &Path, args: &[&str]) -> Output {
    run_ruoyi_with_env(current_dir, args, &[])
}

fn run_ruoyi_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_ruoyi-cli"));
    command.current_dir(current_dir).args(args);
    for (key, value) in envs {
        command.env(key, value);
    }
    command.output().expect("ruoyi should launch")
}
