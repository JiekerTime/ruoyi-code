...
fn main() {
    ...
    if let Err(error) = run() {
        ...
        if message.contains("`ruoyi-cli --help`") {
            eprintln!("error: {message}");
        } else {
            eprintln!(
                "error: {message}

Run `ruoyi-cli --help` for usage."
            );
        }
        std::process::exit(1);
    }
...
/// Load `~/.ruoyi-cli/.env` into process environment (lowest priority).
fn load_global_env() {
    let Some(home) = env::var_os("HOME") else {
        return;
    };
    let path = std::path::Path::new(&home).join(".ruoyi-cli").join(".env");
    load_env_file(&path);
}

/// Load `.env` from the current working directory (overrides global).
fn load_local_env() {
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // Ruoyi Code compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // Ruoyi Code compat: --print makes output non-interactive
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`ruoyi-cli {command_name}` is a slash command. Use `ruoyi-cli --resume SESSION.jsonl /{command_name}` or start `ruoyi-cli` and run `/{command_name}`."
        )
    } else {
        format!(
            "`ruoyi-cli {command_name}` is a slash command. Start `ruoyi` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `ruoyi` and run it there, or use `ruoyi-cli --resume SESSION.jsonl {command_name}` / `ruoyi-cli --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `ruoyi-cli --help` for CLI usage, or start `ruoyi-cli` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("ruoyi-plugin:")
        .then_some(
            "Compatibility note: `/ruoyi-plugin:*` is a Ruoyi Code plugin command. `ruoyi` does not yet load plugin slash commands, Claude statusline stdin, or session hooks.",
        )
}
...
/// Core tools for models with limited context windows (non-Claude).
const SMALL_CONTEXT_TOOLS: &[&str] = &[
    "bash",
    "read_file",
    "write_file",
    "edit_file",
...
/// For non-Claude models with limited context, use a minimal system prompt.
fn compact_system_prompt_for_small_context(_full_prompt: &str) -> String {
    "You are a helpful software engineering assistant. You have access to tools for reading files, writing files, editing files, running shell commands, and searching. Use tools when the user asks you to interact with the filesystem or codebase. Respond concisely.".to_string()
}
...
/// For models with small context windows, auto-limit to core tools only.
fn filter_tool_specs_for_model(
    tool_registry: &GlobalToolRegistry,
    allowed_tools: Option<&AllowedToolSet>,
    model: &str,
) -> Vec<ToolDefinition> {
    // If user explicitly set --allowedTools, respect that.
    if allowed_tools.is_some() {
        return filter_tool_specs(tool_registry, allowed_tools);
    }
    // For non-Claude models with smaller context, use a reduced tool set.
    if !model.starts_with("claude") {
        let all = tool_registry.definitions(None);
        return all
            .into_iter()
            .filter(|tool| SMALL_CONTEXT_TOOLS.contains(&tool.name.as_str()))
            .collect();
    }
    tool_registry.definitions(allowed_tools)
}
...
/// Starts a minimal Model Context Protocol server that exposes ruoyi's
...
/// Read `.ruoyi/worker-state.json` from the current working directory and print it.
/// This is the file-based worker observability surface: `push_event()` in `worker_boot.rs`
/// atomically writes state transitions here so external observers (ruoyi-orchestrator, orchestrators)
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let state_path = cwd.join(".ruoyi").join("worker-state.json");
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        Ok(Some(token_set)) => {
            ...
            if expired {
                details.push(
                    "Suggested action  ruoyi-cli login to refresh local OAuth credentials"
                        .to_string(),
                );
            }
            DiagnosticCheck::new(
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .ruoyi/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
    ...
    match command {
        ...
        SlashCommand::Clear { confirm } => {
            ...
            Ok(ResumeCommandOutcome {
                session: cleared,
                message: Some(format!(
                    "Session cleared\n  Mode             resumed session reset\n  Previous session {previous_session_id}\n  Backup           {}\n  Resume previous  ruoyi-cli --resume {}\n  New session      {new_session_id}\n  Session file     {}",
                    backup_path.display(),
                    backup_path.display(),
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `ruoyi-cli` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
/// commit (from `--base-commit` flag or `.ruoyi-base` file). Emits a warning to
...
impl LiveCli {
   ...
   /         O\n\
  /   (_____/\n\
 /_____/   U\x1b[0m  \x1b[1;38;5;208mRuoyi Code\x1b[0m 🐕\n\n\
  \x1b[2mModel\x1b[0m            {}\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn run_turn(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match result {
            Ok(summary) => {
                ...
                // Ensure streamed text is on its own line before spinner clears the line.
                write!(stdout, "\n").ok();
                spinner.finish(
                    "✨ Done",
                    TerminalRenderer::new().color_theme(),
    ...
    fn print_mcp(
        ...
        // `ruoyi-cli mcp serve` starts a stdio MCP server exposing ruoyi's built-in
...
fn resolve_managed_session_path(session_id: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    // `.ruoyi/sessions/<id>.{jsonl,json}` without the per-workspace hash
    // subdirectory. Walk up from `directory` to the `.ruoyi/sessions/` root
...
fn list_managed_sessions() -> Result<Vec<ManagedSessionSummary>, Box<dyn std::error::Error>> {
    ...
    // flat `.ruoyi/sessions/` root so users do not lose access to existing
...
fn format_missing_session_reference(reference: &str) -> String {
    format!(
        "session not found: {reference}\nHint: managed sessions live in .ruoyi/sessions/. Try `{LATEST_SESSION_REFERENCE}` for the most recent session or `/session list` in the REPL."
    )
}

fn format_no_managed_sessions() -> String {
    format!(
        "no managed sessions found in .ruoyi/sessions/\nStart `ruoyi` to create a session, then rerun with `--resume {LATEST_SESSION_REFERENCE}`."
    )
}
...
fn render_repl_help() -> String {
    [
        ...
        "  Auto-save            .ruoyi/sessions/<session-id>.jsonl".to_string(),
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Related          /status · ruoyi --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            ruoyi sandbox
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Related          /sandbox · ruoyi status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            ruoyi-cli doctor
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Related          /doctor · ruoyi --resume latest /doctor"
            .to_string(),
    }
}
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // (`crates/ruoyi-cli/tests/compact_output.rs`) to point
        // ruoyi at its fake Anthropic endpoint. We also attach a
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        let message_request = MessageRequest {
            ...
            system: (!request.system_prompt.is_empty()).then(|| {
                let full = request.system_prompt.join("\n\n");
                if self.model.starts_with("claude") {
                    full
                } else {
                    compact_system_prompt_for_small_context(&full)
                }
            }),
            tools: self.enable_tools.then(|| {
                filter_tool_specs_for_model(
                    &self.tool_registry,
                ...
                ApiStreamEvent::ContentBlockDelta(delta) => match delta.delta {
                    ContentBlockDelta::TextDelta { text } => {
                        if !text.is_empty() {
                            ...
                            // Clear spinner line before first text output
                            if !cleared_spinner {
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   ruoyi-cli --resume {session_id} /compact"
    ));
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "ruoyi-cli v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  ruoyi [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  ruoyi [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  ruoyi [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  ruoyi-cli --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  ruoyi help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  ruoyi-cli version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  ruoyi status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  ruoyi sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  ruoyi-cli doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  ruoyi dump-manifests")?;
    writeln!(out, "  ruoyi bootstrap-plan")?;
    writeln!(out, "  ruoyi-cli agents")?;
    writeln!(out, "  ruoyi-cli mcp")?;
    writeln!(out, "  ruoyi skills")?;
    writeln!(
        out,
        "  ruoyi system-prompt [--cwd PATH] [--date YYYY-MM-DD]"
    )?;
    writeln!(out, "  ruoyi-cli login")?;
    writeln!(out, "  ruoyi logout")?;
    writeln!(out, "  ruoyi-cli init")?;
    writeln!(
        out,
        "  ruoyi-cli export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .ruoyi/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  ruoyi-cli --model claude-opus \"summarize this repo\""
    )?;
    writeln!(
        out,
        "  ruoyi-cli --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  ruoyi-cli --compact \"summarize Cargo.toml\" | wc -l"
    )?;
    writeln!(
        out,
        "  ruoyi-cli --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  ruoyi-cli --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  ruoyi-cli --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  ruoyi-cli agents")?;
    writeln!(out, "  ruoyi-cli mcp show my-server")?;
    writeln!(out, "  ruoyi-cli /skills")?;
    writeln!(out, "  ruoyi-cli doctor")?;
    writeln!(out, "  ruoyi-cli login")?;
    writeln!(out, "  ruoyi-cli init")?;
    writeln!(out, "  ruoyi-cli export")?;
    writeln!(out, "  ruoyi-cli export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   ruoyi-cli --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("ruoyi-cli-{nanos}-{unique}"))
    }
    ...
    fn write_plugin_fixture(root: &Path, name: &str, include_hooks: bool, include_lifecycle: bool) {
        fs::create_dir_all(root.join(".ruoyi-plugin")).expect("manifest dir");
        if include_hooks {
        ...
        fs::write(
            root.join(".ruoyi-plugin").join("plugin.json"),
            format!(
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".ruoyi")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".ruoyi").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        .expect("project config should write");

        let original_config_home = std::env::var("RUOYI_CONFIG_HOME").ok();
        let original_permission_mode = std::env::var("RUSTY_CLAUDE_PERMISSION_MODE").ok();
        std::env::set_var("RUOYI_CONFIG_HOME", &config_home);
        std::env::remove_var("RUSTY_CLAUDE_PERMISSION_MODE");

        let resolved = with_current_dir(&cwd, super::default_permission_mode);

        match original_config_home {
            Some(value) => std::env::set_var("RUOYI_CONFIG_HOME", value),
            None => std::env::remove_var("RUOYI_CONFIG_HOME"),
        }
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("ruoyi-cli --resume SESSION.jsonl /status"));
    }
    ...
    fn formats_namespaced_omc_slash_command_with_contract_guidance() {
        let report = format_unknown_slash_command_message("ruoyi-plugin:hud");
        assert!(report.contains("unknown slash command: /ruoyi-plugin:hud"));
        assert!(report.contains("Ruoyi Code plugin command"));
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("ruoyi-cli --help"));
    }
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .ruoyi/sessions/<session-id>.jsonl"));
    ...
    fn resolve_repl_model_falls_back_to_anthropic_model_env_when_default() {
        ...
        std::env::remove_var("ANTHROPIC_MODEL");
        std::env::remove_var("RUOYI_CONFIG_HOME");
        fs::remove_dir_all(root).expect("cleanup temp dir");
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("ruoyi-cli help"));
        assert!(help.contains("ruoyi-cli version"));
        assert!(help.contains("ruoyi-cli status"));
        assert!(help.contains("ruoyi-cli sandbox"));
        assert!(help.contains("ruoyi-cli init"));
        assert!(help.contains("ruoyi-cli agents"));
        assert!(help.contains("ruoyi-cli mcp"));
        assert!(help.contains("ruoyi-cli skills"));
        assert!(help.contains("ruoyi-cli /skills"));
    }
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("ruoyi-cli --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("ruoyi-cli --resume latest"));
        assert!(help.contains("ruoyi-cli --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".ruoyi/sessions/legacy.json");
    ...
    fn unknown_omc_slash_command_guidance_explains_runtime_gap() {
        let message = format_unknown_slash_command("ruoyi-plugin:hud");
        assert!(message.contains("Unknown slash command: /ruoyi-plugin:hud"));
        assert!(message.contains("Ruoyi Code plugin command"));
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".ruoyi/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("ruoyi-cli-{label}-{nanos}"))
    }
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/ruoyi-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p ruoyi-cli"}"#)
                .contains("cargo test -p ruoyi-cli")
        );
        assert_eq!(
