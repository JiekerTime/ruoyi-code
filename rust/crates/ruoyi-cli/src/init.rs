...
const STARTER_RUOYI_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Ruoyi Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".ruoyi/settings.local.json", ".ruoyi/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let ruoyi_dir = cwd.join(".ruoyi");
    artifacts.push(InitArtifact {
        name: ".ruoyi/",
        status: ensure_dir(&ruoyi_dir)?,
    });

    let ruoyi_json = cwd.join(".ruoyi.json");
    artifacts.push(InitArtifact {
        name: ".ruoyi.json",
        status: write_file_if_missing(&ruoyi_json, STARTER_RUOYI_JSON)?,
    });
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Ruoyi Code (ruoyicode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.ruoyi.json`; reserve `.ruoyi/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("ruoyi-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".ruoyi/"));
        assert!(rendered.contains(".ruoyi.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".ruoyi").is_dir());
        assert!(root.join(".ruoyi.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".ruoyi.json")).expect("read ruoyi json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".ruoyi/settings.local.json"));
        assert!(gitignore.contains(".ruoyi/sessions/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".ruoyi/settings.local.json\n")
        ...
        assert!(second_rendered.contains(".ruoyi/"));
        assert!(second_rendered.contains(".ruoyi.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".ruoyi/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".ruoyi/sessions/").count(), 1);
