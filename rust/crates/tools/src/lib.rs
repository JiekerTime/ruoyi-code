...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("ruoyid-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(ruoyi_config_home) = std::env::var("RUOYI_CONFIG_HOME") {
        push_prefixed_skill_lookup_roots(&mut roots, std::path::Path::new(&ruoyi_config_home));
    }
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.ruoyi/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".ruoyi"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".ruoyi"));
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("ruoyid-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    ...
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".ruoyid-agents"));
    }
    Ok(cwd.join(".ruoyid-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "ruoyiguide" | "ruoyiguideagent" | "guide" => String::from("ruoyi-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".ruoyi").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("RUOYI_CONFIG_HOME") {
        return Ok(PathBuf::from(path));
    }
    let home = std::env::var("HOME").map_err(|_| String::from("HOME is not set"))?;
    Ok(PathBuf::from(home).join(".ruoyi"))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("ruoyid-tools-{unique}-{name}"))
    }
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .ruoyi/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let ruoyi_dir = worktree.join(".ruoyi");
        fs::create_dir_all(&ruoyi_dir).expect("create .ruoyi dir");
        ...
        fs::write(ruoyi_dir.join("settings.json"), settings).expect("write settings");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .ruoyi/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".ruoyi").join("worker-state.json");
    ...
    fn stall_detect_and_restart_recovery_end_to_end() {
        // Worker stalls at trust_required, ruoyi-orchestrator restarts instead of resolving
        let created = execute_tool(
            "WorkerCreate",
    ...
    fn skill_loads_local_skill_prompt() {
        ...
        fs::write(
            ...
            "# help\n\nGuide on using ruoyi-codex plugin\n",
        )
        ...
        assert!(output["prompt"]
            .as_str()
            .expect("prompt")
            .contains("Guide on using ruoyi-codex plugin"));
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".ruoyi").join("skills").join("plan");
        let command_dir = root.join(".ruoyi").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".ruoyi/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".ruoyi/commands/handoff.md"));
    ...
    fn skill_loads_project_local_claude_skill_prompt() {
        ...
        let skill_dir = workspace.join(".ruoyi").join("skills").join("trace");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(output["path"]
            .as_str()
            .expect("path")
            .ends_with(".ruoyi/skills/trace/SKILL.md"));
    ...
    fn skill_loads_project_local_legacy_command_markdown() {
        ...
        let workspace = root.join("workspace");
        let nested = workspace.join("nested");
        let command_dir = workspace.join(".ruoyi").join("commands");
        ...
        assert!(output["path"]
            .as_str()
            .expect("path")
            .ends_with(".ruoyi/commands/team.md"));
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".ruoyi")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".ruoyi")).expect("cwd dir");
        std::fs::write(
            home.join(".ruoyi").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".ruoyi")).expect("cwd dir");
        std::fs::write(
            cwd.join(".ruoyi").join("settings.local.json"),
        ...
        let local_settings =
            std::fs::read_to_string(cwd.join(".ruoyi").join("settings.local.json"))
                .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".ruoyi").join("tool-state").join("plan-mode.json"))
...
"#,
    ...
    fn run_task_packet_creates_packet_backed_task() {
        let result = run_task_packet(TaskPacket {
            ...
            repo: "ruoyi-code-parity".to_string(),
        ...
        assert_eq!(output["task_packet"]["repo"], "ruoyi-code-parity");
        assert_eq!(
